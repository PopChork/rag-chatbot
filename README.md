...
The `data/uploads/` folder may contain private lecture notes or copyrighted course material. These files should not be committed to a public GitHub repository.

The `.gitignore` file is configured to ignore uploaded documents and generated review CSVs. Before publishing the repository, double-check that no private PDFs, extracted lecture text, or generated review files containing lecture-note content are committed.
...
Remove Python cache files if needed:
...
- Answer quality depends on the local Ollama model being used.
- PDF extraction quality affects downstream retrieval quality.
- The system currently uses character-based chunking rather than semantic or layout-aware chunking.
- The `/query` endpoint retrieves across the full collection and does not currently expose a `doc_id` filter.
- Faithfulness review is manual rather than fully automated.
- The API does not include authentication or user-level document isolation, so it is intended for local development and experimentation rather than public deployment.

## Possible Improvements

- Add a frontend chat interface
- Add document deletion and collection reset endpoints
- Add `doc_id` filtering to the `/query` endpoint
- Add hybrid retrieval using dense vectors plus keyword search
- Add reranking before answer generation
- Add automated faithfulness checks using an LLM-as-judge workflow
- Add authentication before deploying beyond local use
- Add tests for document ingestion, retrieval, and API response formats

## Resume Summary

This project demonstrates an end-to-end RAG workflow: document ingestion, chunking, embedding generation, vector search, grounded LLM answer generation, Dockerized local deployment, and retrieval evaluation using Recall@k, MRR, and latency.
